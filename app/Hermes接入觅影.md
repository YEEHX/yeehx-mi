...
mcp_servers:
  miying:
    command: "/Users/你的用户名/Dev/YEEHX-Mi/app/mcp_run.sh"   # Windows 填 mcp_run.bat 的完整路径
    args: []
    allowed_tools:
      - miying_search
      - miying_preview
...
Windows 机器上跑觅影时改成（路径换成你的解压位置）：

```json
{
  "type": "stdio",
  "command": "cmd",
  "args": ["/c", "D:\\MiYing\\app\\mcp_run.bat"]
}
```
...
```yaml
    env:
      YEEHX_MI_PORT: "8788"            # 觅影端口，跟启动脚本（.command/.bat）保持一致
      MIYING_WECHAT_LIMIT_MB: "950"    # 超过这个体积就转代理片（微信上限 1GB，留余量）
```

## 八、注意

- **电脑别睡死**：mac 在 系统设置 → 电池/能源 关闭「自动进入睡眠」（或常驻 `caffeinate -s`）；Windows 在 设置 → 系统 → 电源 把「睡眠」设为从不。机器睡了整条链路都断。
...
| 现象 | 看哪 |
| --- | --- |
| /tools 里没有 miying | `hermes doctor`；确认 config.yaml 路径正确、`mcp_run.sh` 有执行权限（`chmod +x`）；Windows 用 mcp_run.bat |
| 提示「没找到 app/.venv」 | 先双击跑一次 `启动觅影.command`（Windows：`启动觅影.bat`）装环境 |
