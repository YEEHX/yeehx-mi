...
- **「我的电脑很老了 / 我不懂 Python，装得上吗？」** —— 不挑机器也不用懂：老 Intel Mac、无独显 Windows 一样跑（AI 走 API 模式）；缺 Python 会自动下载便携版进觅影自己的文件夹，全程不需要你操作、不碰系统。
...
>
> **系统要求**：macOS 或 Windows 10/11，**其他什么都不用装**——缺 Python 时启动脚本会自动下载一个便携版（国内镜像，约 24MB），只放进觅影自己的文件夹，不碰你的系统，删文件夹即彻底卸载。
> Apple Silicon / Intel Mac 都支持；Intel Mac 与无独显 Windows 机器建议 AI 走 API 模式。

## 老用户升级（不丢库）

从 v2.1.1 起升级是全自动的：下载新版 zip 解压 → 双击新文件夹里的启动脚本。后台还挂着的旧版本服务会被自动停掉换新；首次进入时觅影会发现旧文件夹里的素材库，提示**一键搬家**——库、标签、星级、设置全部保留，不用重扫。确认没问题后，旧文件夹随时可删。

万一自动没发现（比如旧文件夹被挪去了别的盘）：手动把旧文件夹的 `app/out` 整个拷进新文件夹的 `app/` 下，再启动即可。
