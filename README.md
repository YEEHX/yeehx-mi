...
本地运行的 AI 影视素材库 —— 扫描硬盘 → AI 理解素材 → 用人话搜索 → 一键调用进剪辑。

由影像创作者 [玩椰](https://www.yeehx.com) 为自己十几 TB 的素材库而写。AI 时代应该知识平权——所以它开源，分享给每一个被素材淹没的创作者。

[官网下载](https://www.yeehx.com/miying) · [图文使用说明](docs/使用说明.md) · [3 分钟上手视频](https://www.yeehx.com/miying) · [更新日志](app/CHANGELOG.md)
...
> 顶栏输入 **「武汉 江边 日落 航拍」** → 结果直接出来 → 选中 → 导出到剪辑工程，或在 Finder 里定位原片。
...
- **像 Finder 一样浏览**：外挂硬盘直接进，每段素材一张代表帧、零延迟速览，一眼看清每段大概拍了什么；LUT 还原 log 灰片、星级选片、键盘流操作
- **原素材永远只读**：绝不重命名 / 移动 / 删除 / 改写任何文件；一切数据只写进自己的 `app/out/`，删掉即彻底卸载
- **离线优先**：全部在你的 Mac 上跑。唯一的出站请求是启动时向官网查一次新版本号，设置里可关
...
2. 双击 `app/启动觅影.command`（首次会自动装依赖，约 1-3 分钟；之后秒开）
...
>
> **系统要求**：macOS + Python ≥ 3.10（没有会引导安装）。Apple Silicon / Intel 都支持，Intel 机器建议 AI 走 API 模式。
> **Windows**：移植中，关注 [玩椰](#关注玩椰) 第一时间获取。

## AI 模型怎么选

| 方案 | 适合 | 说明 |
|---|---|---|
| 本地 Ollama | M 系列芯片、在意隐私 | 装 [Ollama](https://ollama.com) 拉一个视觉模型（推荐 Qwen3.6 A3B，24GB+ 内存更从容），完全免费、全程不出网 |
| OpenAI 兼容 API | Intel Mac、要速度 | 填地址 + Key（如小米 MiMo `platform.xiaomimimo.com`），12 路并发打标，大库一晚上打完 |
| 先不配 | 只想先用起来 | 浏览 / 扫描 / LUT / 手动标签 / 按词搜索全部可用，AI 打标随时补 |

## 主要功能

浏览与选片（LUT 预览、星级、键盘流、大图 lightbox、换封面帧）；扫描与理解（快扫建底稿、精扫、同步对账增量更新、搬家识别、重复检测）；查找与调用（自然语言 AI 找、标签 / 星级 / LUT 筛选、全文搜索、导出复制、Finder 定位、手机局域网访问）；词表治理（类目管理、别名、合并建议、参考图、低频清理、备份还原）；以及 MCP 接口——可以把觅影接进支持 MCP 的 AI 助手，用对话管理素材库（见 `app/Hermes接入觅影.md`）。
